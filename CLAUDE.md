...
- [FILE-INDEX.md](FILE-INDEX.md) is a one-line-per-file description of everything already in `source-manuals/` and `assets/` — check it before telling the user a document doesn't exist. It's a static snapshot (not regenerated automatically); update the relevant section after adding/renaming files in a batch, rather than letting it drift silently. When describing a document's *type* (install guide vs. product page vs. sell sheet), verify against the file's actual content or its existing link `label:` in `index.html` — don't infer type from the original filename alone, which is frequently wrong (e.g. a file named `PALPCR-1Z.pdf` turned out to be a 2-page marketing sheet, not an install guide).
